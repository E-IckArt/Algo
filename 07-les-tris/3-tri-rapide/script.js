function tri_rapide(tableau_a_trier) {
    if (tableau_a_trier.length <= 1) {
        ...
        for (const element of tableau_a_trier) {
            if (element < pivot) {
                tab_plus_petites_valeurs.push(element);
            }
        }
        ...
        return [].concat(
            tri_rapide(tab_plus_petites_valeurs),
            [pivot],
            tri_rapide(tab_plus_grandes_valeurs)
        );
    }
}
console.log(tri_rapide([2000, 1998, 2016, 2003, 2020, 1992, 2005]));
